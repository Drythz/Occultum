...
//=============================================================================
 /*:
 * @plugindesc v1.14 Permite que o sistema de equipamento seja mais
 * flexível para permitir slots de equipamentos únicos por classe.
 ...
 *
 * @param Text Align
 * @desc Como alinhar o texto para a janela de comando.
 * left     center     right
 * @default center
 *
 * @param Finish Command
 * @desc O texto de comando usado para sair da cena de equipamento.
 * @default Finish
 *
 * @param Remove Text
 * @desc O texto usado para mostrar o comando "Remove" na lista de equipamento
 * de item.
 * @default Remove
 *
 * @param Remove Icon
 * @desc O ícone usado para mostrar próximo do comando "Remove" na lista de
 * equipamento de item.
 * @default 16
 *
 * @param Empty Text
 * @desc O texto usado para mostrar um pedaço de equipamento "Empty".
 * @default <Empty>
 *
 * @param Empty Icon
 * @desc  O ícone usado para mostrar próximo do pedaço de equipamento "Empty"
 ...
 * @desc Esses tipos têm que ter sempre um item equipado e não podem estar
 * vazios. Separe os tipos ID's por um espaço.
 * @default 1
 *
 * @param Non-Optimized Types
 * @desc Esses tipos serão ignorados quando o personagem otimizar equipamentos.
 * Separe os tipos ID's por um espaço.
 * @default 5
 *
 * @help
 * ============================================================================
 ...
 * ============================================================================
 *
 * Esse plugin altera vários aspectos em relação ao manuseio de equipamento.
 * As mudanças são as seguintes:
 *
 * 1. Scene_Equip
 * Scene_Equip foi modificado para parecer diferente. Isso é feito
 * primeiramente para fazer as cenas do menu principal paracerem uniformes e
 * manterem tudo familiarizado para jogadores. Além disso, a janela de comando
 * foi ajustada para ser melhor encaixada por plugins de extensão no futuro
 * que podem adicionar comandos na janela de comando e/ou a cena.
 *
 * 2. Manuseio de Tipo de Equipamento
 * Personagens não terão mais um setting de slot de equipamento universal.
 * Agora, classes diferentes podem usar diferentes setups por simplesmente
 * adicionarem alguns poucos notetags para o notebox da classe. Além disso,
 * tipos de equipamentos no passado com nomes iguais seriam tratados como
 * tipos separados. Agora, tipos de equipamentos com nomes iguais serão
 * tratados como o mesmo tipo.
 *
 * 3. Regras de Equipamento
 * Agora, certos tipos de equipamentos podem ou não ser removidos. Por
 * exemplo, esse plugin pode estabelecer de forma que o slot de Arma tem que
 * ter sempre alguma coisa equipada e que o jogador não pode manualmente
 * deixá-lo vazio (o jogo, por outro lado, pode realizar isso através de
 * eventos). Além disso, otimizar equipamentos pode ser restrito para certos
 * tipos de equipamentos, que são melhores se decididos manualmente (como
 * acessórios).
 *
 * 4. Controle de Parâmetro
 * Parâmetros de equipamentos agora podem ser ajustados através de notetags
 * para terem um valor grande ou valor personalizado (através de código). Isso
 * permite para equipamentos a não serem mais itens estáticos, mas em vez
 * disso, equipamentos podem agora ser dinâmicos e podem mudar ao longo do
 * jogo.
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 *
 * Você pode usar os seguintes notetags para mudar o setup de equipamento
 * de uma classe.
 *
 * Notetags de Classe:
 *   <Equip Slot: x>      Exemplo: <Equip Slot: 1, 2, 3, 4, 5, 5, 5, 5>
 *   <Equip Slot: x, x, x>
 *   Muda os slots de equipamento dessa classe para x. Usar números repetidos
 *   faz com que tipos de equipamentos sejam duplicados e que a classe pode
 *   equipar múltiplos equipamentos daquele tipo. Para achar o ID do tipo
 *   do equipamento, vá para a aba de Tipo no seu banco de dados e proucure
 *   para o tipo de ID.
 *
 *   Se você não gostar do método acima para personalizar slots de
 *   equipamento, você pode usar os seguintes notetags em vez disso:
 *
 *   <Equip Slot>         Exemplo: <Equip Slot>
 *    string                        Weapon
 *    string                        Armor
 *    string                        Accessory
 *    string                        Accessory
 *   </Equip Slot>                 </Equip Slot>
 *   Substitua 'string' com o nome de entrada to tipo de equipamento.
 *   Maiúsculas e minúsculas fazem a diferença aqui, então se o string não
 *   combinar exatamente com o nome de entrada, o slot não será concedido
 *   para a classe. Cópias múltiplas de nomes de entrada quer dizer que a
 *   classe pode equipar equipamentos múltiplos daquele tipo. Tudo funciona
 *   da mesma forma que o notetag prévio.
 *
 * Notetags de Arma e Armadura:
 *   <stat: +x>
 *   <stat: -x>
 *   Permite o pedaço de arma ou armadura a ganhar ou perder x quantia de
 *   status. Substitua "stat" por "hp", "mp", "atk", "def", "mat", "mdf",
 *   "agi", ou "luk" para alterar o status específico. Isso permite o
 *   pedaço de equipamento a ir além da limitação padrão do editor desde que
 *   o valor máximo o permita para isso. Mudanças feitas aqui alteram os
 *   parâmetros da base.
 *
 * ============================================================================
 * Modo Lunático - Parâmetros Personalizados
 * ============================================================================
 *
 *   <Custom Parameters>  Exemplo: <Custom Parameters>
 *    código                        atk = $gameVariables.value(1);
 *    código                        mat = atk / 2;
 *    código                        all = $gameParty.members().length;
 *    código                       </Custom Parameters>
 *   </Code Parameters>
 *   Permite os parâmetros a terem taxas personalizadas ajustadas por código.
 *   Os seguintes parâmetros são definidos: 'maxhp', 'maxmp', 'atk', 'def',
 *   'mat', 'mdf', 'agi', 'luk', e 'all'. O parâmetro 'all' irá afetar todos
 *   os parâmetros. Mudanças feitas aqui não alteram os parâmetros da base,
 *   mas em vez disso, são adicionadas nos parâmetros da base.
 *
 * ============================================================================
 * Changelog
 * ============================================================================
